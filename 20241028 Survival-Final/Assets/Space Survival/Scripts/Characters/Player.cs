...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //Componenet 활성화/비활성화 : enavled = bool;
        //두 경우 모두 OnEnabled / OnDisabled 메시지 함수가 호출됨
        ...
        {
            GameObject skillobj = Instantiate(skill.skillPrefab[skill.skillLevel], transform, false); //스킬 오브젝트 생성
            skillobj.name = skill.skillName; //오브젝트 이름 변경
            skillobj.transform.localPosition = Vector2.zero; //스킬 위치를 플레이어의 위치로
    ...
    public void OnLevelUp()
    {
        ...
        if (level < levelupSteps.Length) //최대 레벨에 도달하지 않았다면
        {
    ...
    //파라미터로 넘어온 스킬의 레벨을 상승 시키고, 다음 레벨의 프리팹으로 교체 
    public void OnSkillLevelUp(Skill skill)
    {
        if (skill.skillLevel >= skill.skillPrefab.Length - 1)
        {
            //유효하지 않은 스킬이 넘어올 때
            Debug.LogWarning($"최대 레벨에 도달한 스킬 레벨업을 시도함. {skill.skillName}");
            return;
        }

        skill.skillLevel++; //스킬 레벨 상승

        Destroy(skill.currentSkillObject); //기존에 있던 스킬 오브젝트를 제거함
        //게임 오브젝트 , 부모 , 월드 좌표
